program main
    ...
    character(100) :: line
    character(20) :: num, ctmp2, ctmp3, ctmp4, cnpats
    integer n, m, i, j, tmp, err
    
    real(8),external :: ZBQLUAB
    real(8) tmp2, tmp3, tmp4
    ...
    do n = 1, npats
       ...
       tmp2 = 52.1 + ZBQLUAB(dble(-0.8), dble(0.8))
       tmp3 = 65.1 + ZBQLUAB(dble(-0.8), dble(0.8))
       tmp4 = 76.3 + ZBQLUAB(dble(-0.8), dble(0.8))
       write(ctmp2,'(F10.2)') tmp2
       write(ctmp3,'(F10.2)') tmp3
       write(ctmp4,'(F10.2)') tmp4
       ...
       do m = 1, 5
           ...
           if (err .ne. 0) exit
           if (m .eq. 2) then
              line = "CELL "//trim(adjustl(ctmp2))//" "//trim(adjustl(ctmp3))//" "//trim(adjustl(ctmp4))//" 100.2 111.4 105.8"
           end if
           write(21,'(a)') trim(line)
       end do
       close(20)
       ...
       write(30,*) "CELL ", tmp2, tmp3, tmp4, 100.2, 111.4, 105.8
