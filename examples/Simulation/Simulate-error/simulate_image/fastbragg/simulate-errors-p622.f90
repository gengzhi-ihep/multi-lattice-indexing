program main
    ...
    do n = 1, npats
       ...
       tmp2 = 110.62 + ZBQLUAB(dble(-0.8), dble(0.8))
       tmp3 = 335.83 + ZBQLUAB(dble(-0.8), dble(0.8))
       write(ctmp2,'(F10.2)') tmp2
       write(ctmp3,'(F10.2)') tmp3
       ...
       do m = 1, 5
           ...
           if (err .ne. 0) exit
           if (m .eq. 2) then
              line = "CELL "//trim(adjustl(ctmp2))//" "//trim(adjustl(ctmp2))//" "//trim(adjustl(ctmp3))//" 90.0 90.0 120.0"
           end if
           write(21,'(a)') trim(line)
       end do
       close(20)
       ...
       write(30,*) "CELL ", tmp2, tmp2, tmp3, 90.0, 90.0, 120.0
