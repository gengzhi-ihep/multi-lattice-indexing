program main
    ...
    do n = 1, npats
       ...
       tmp2 = 60.57 + ZBQLUAB(dble(-0.8), dble(0.8))
       tmp3 = 119.75 + ZBQLUAB(dble(-0.8), dble(0.8))
       tmp4 = 170.61 + ZBQLUAB(dble(-0.8), dble(0.8))
       write(ctmp2,'(F10.2)') tmp2
       write(ctmp3,'(F10.2)') tmp3
       ...
       do m = 1, 5
           ...
           if (err .ne. 0) exit
           if (m .eq. 2) then
              line = "CELL "//trim(adjustl(ctmp2))//" "//trim(adjustl(ctmp3))//" "//trim(adjustl(ctmp4))//" 90.0 90.0 90.0"
           end if
           write(21,'(a)') trim(line)
       end do
       close(20)
       ...
       write(30,*) "CELL ", tmp2, tmp3, tmp4, 90.0, 90.0, 90.0
