program main
    ...
    do n = 1, npats
       ...
       tmp2 = 181.13 + ZBQLUAB(dble(-0.8), dble(0.8))
       write(ctmp2,'(F10.2)') tmp2
       ...
       do m = 1, 5
           ...
           if (err .ne. 0) exit
           if (m .eq. 2) then
              line = "CELL "//trim(adjustl(ctmp2))//" "//trim(adjustl(ctmp2))//" "//trim(adjustl(ctmp2))//" 90.0 90.0 90.0"
           end if
           write(21,'(a)') trim(line)
       end do
       close(20)
       ...
       write(30,*) "CELL ", tmp2, tmp2, tmp2, 90.0, 90.0, 90.0
